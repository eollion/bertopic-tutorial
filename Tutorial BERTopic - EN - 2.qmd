...
# Understanding BERTopic's philosophy <!-- mehh philosophy -->
...
- Generates a digital or mathematical **XXX** representation that captures the semantic properties of each document — the embeddings.
...
If you look in the documentation, these three steps are composed of technical blocks, each of them connected to a data treatment. <!--  meh -->
...
<!-- AM: 
I wrote : This is a normal behaviour of the clustering algorithm as it focuses on dense clusters first. 
ES changed to : This is a normal behaviour of the clustering algorithm as it defines cluster based on spot of proximity between documents

Is the disagreement on the formulation or the content?
-->
...
A good way to visualise how topics interact with each other is to plot a dendogram. This plot is read from left to right, the sooner branches merge, the closer related topics are. We se the `visualize_hierarchy` method. The graph is very tall but we can easily focus on one subset of the tree at a time.
